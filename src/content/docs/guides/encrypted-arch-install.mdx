...
lsblk -f | grep home >> /etc/fstab # now you have the UUID at the bottom of the file
nvim /etc/fstab

UUID=**ROOT UUID FROM BOTTOM OF FILE**      /      ext4      rw,relatime      0 1
UUID=**HOME UUID FROM BOTTOM OF FILE**      /      ext4      rw,relatime      0 1
```
...
lsblk -f | grep nvme0n1p3 >> /etc/crypttab # now you have the UUID at the bottom of the file
nvim /etc/crypttab
